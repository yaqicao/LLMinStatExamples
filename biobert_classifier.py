...
try:
    ...
    print(f"出错，文件未找到！检查'{file_path}'文件与Python脚本在同一个文件夹下。")
    exit()
except KeyError:
    print(f"出错，找不到'note'或'department'列。")
    exit()
...
X_train, X_test, y_train, y_test = train_test_split(X_pca_30, labels, test_size=0.25, random_state=42, stratify=labels)
...
explained_variance = pca_2.explained_variance_ratio_ * 100
plt.figure(figsize=(12, 8))
colors = ['#1f77b4', '#ff7f0e'] 
for label_num, department_name in label_to_department_name.items():
    plt.scatter(X_pca_2[labels == label_num, 0], 
                X_pca_2[labels == label_num, 1], 
                c=colors[label_num], 
                label=department_name,
                alpha=0.8)
plt.title('BioBERT病历文本分类可视化', fontsize=20)
plt.xlabel(f'主成分 1 ({explained_variance[0]:.2f}%)', fontsize=16)
plt.ylabel(f'主成分 2 ({explained_variance[1]:.2f}%)', fontsize=16)
