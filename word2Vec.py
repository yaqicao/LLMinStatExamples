...
# --- 1. Five-sentence clinical case report corpus ---
sentences = [
...
try:
    ...
    word_to_check = '患者'
    # Obtain word vector (a 100-dimensional NumPy array)
    word_vector = model.wv[word_to_check]
    # Use slicing to obtain the first five dimensions
...
try:
    ...
    plt.title("Word2Vec 词向量空间 ", fontsize=28)
    plt.xlabel("主成分一", fontsize=22)
    plt.ylabel("主成分二", fontsize=22)
    plt.tick_params(axis='both', which='major', labelsize=16)
    plt.grid(True, linestyle='--', alpha=0.6)

    plt.savefig("word_vectors_final.png", dpi=300, bbox_inches='tight')
    print("\n已生成最终稳定版结果: word_vectors_final.png")
   


except ImportError:
    print("\n【错误】可视化需要安装 matplotlib, scikit-learn 和 adjustText 库。")
